...
namespace UdpTest
{
    public class GameState
    {
        ...
        public Player AddPlayer(ushort id)
        {
            ...
            Players[^1] = new Player(id);
            return Players[^1];
        }

        public void MovePlayer(ushort id, short x, short y)
        {
            for (int i = 0; i < Players.Length; i++)
            {
        ...
        public static GameState ReadState(BinaryReader reader)
        {
            ...
            for (int i = 0; i < playerCount; i++)
            {
                try
                {
                    ...
                    if (gameState.delta.Get(n++))
                        gameState.Players[i].x = reader.ReadInt16();
                    if (gameState.delta.Get(n++))
                        gameState.Players[i].y = reader.ReadInt16();
                    if (gameState.delta.Get(n++))
                        gameState.Players[i].health = reader.ReadByte();
                }
                catch (EndOfStreamException)
                {
                    Debug.LogError($"End of stream for {i}");
                }
            }
